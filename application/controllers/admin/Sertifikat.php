...
class Sertifikat extends CI_Controller
{
    ...
    {
        $data["sertifikat"] = $this->sertifikat_model->getTable4();
        $data["sertifikat3"] = $this->sertifikat_model->getTable();
        $data["sertifikat2"] = $this->sertifikat_model->getTable1();
        $data['current_user'] = $this->auth_model->current_user();
		$this->load->view("admin/sertifikat/list", $data);
    }
    ...
    public function add()
    {
		...
		if ($validation->run()) {
			...
			redirect(site_url('admin/sertifikat'));
        }

        $data['program_studi']=$this->auth_model->prodi_enums('user','program_studi');
		$data['current_user'] = $this->auth_model->current_user();
		$this->load->view("admin/sertifikat/add", $data);
    }
    ...
    public function print($id = null)
	{
		...
		$html = $this->load->view('admin/sertifikat/cetak', $this->data, true);	    
    ...
    public function edit($id = null)
    {
        ...
        if (!isset($id)) redirect('admin/sertifikat');
